...
$base_ipc_dir = "../RESULTS/G.1C.8MB.S0/" ;
$stat      = "AVG_CORE_CYCLES";
$wsuite    = "lab1";
$max_dirs  = 1024;
$norm_dir  = -1;
...
print(STDERR "\t-shamt <val>           : shift stat by an amount equal to shamt \n");
...
print(STDERR "\t-gmean or -g           : print geometric mean. \n");
print(STDERR "\t-hmean                 : print harmonic mean. \n");

print(STDERR "\t-invert or -i          : print inverted values (useful for CPI) \n");
...
invert_stats() if($invert);
multiply_stats() if($mstat);
shift_stats() if($shamt);
  ...
  if($norm_dir != -1){
    ...
    for($ii=0; $ii< $num_w; $ii++){
	  ...
	  if($norm_val != 0){
	      ...
	      $data[$dirnum][$ii] = 0; # To Avoid DivideByZero
	  }
      }
    }
...
sub remap_wname{
  ...
  $wname =  "BC"          if($wname eq "bc-bc-bc-bc-bc-bc-bc-bc");
  $wname =  "BreadthFS"   if($wname eq "bfs-bfs-bfs-bfs-bfs-bfs-bfs-bfs");
  $wname =  "ConnComp"   if($wname eq "cc-cc-cc-cc-cc-cc-cc-cc");
  $wname =  "PageRank"   if($wname eq "pr-pr-pr-pr-pr-pr-pr-pr");
  $wname =  "SSSPath"   if($wname eq "sssp-sssp-sssp-sssp-sssp-sssp-sssp-sssp");
...
for($dirnum=0; $dirnum < @dirs; $dirnum++){
    $dir_sums[$dirnum]=0;
    for($ii=0; $ii< $num_w; $ii++){
	($dir_sums[$dirnum] += (1/$data[$dirnum][$ii])) if($data[$dirnum][$ii]);
    }
}
...
sub shift_stats{
  $shift_factor = eval($shamt);

  for($dirnum=0; $dirnum<@dirs; $dirnum++){
    for($ii=0; $ii< $num_w; $ii++){
      $data[$dirnum][$ii] -= $shift_factor;
    }
  }
}
...
sub invert_stats{
  ...
  for($dirnum=0; $dirnum<@dirs; $dirnum++){
    for($ii=0; $ii< $num_w; $ii++){
      $data[$dirnum][$ii] = 1/$data[$dirnum][$ii] if($data[$dirnum][$ii]);
    }
  }
...
sub get_base_ipc{
	...
	while( ($breadable) && ($_ = <BIN>) ){
	    @bwords = split/\s+/, $_;
	    if($bwords[0] && $bwords[0] eq "CORE_00_IPC"){
		$retval = $bwords[2];
		$bfound=1;
	    }
	}
